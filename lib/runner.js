...
var url = require('url');
var updateUsage = require('minigun-profiler');
...
// Instantiate Histogram for cpu and memory
  if (typeof script.pid === 'number') {
    intermediate._collection.cpu = new Measured.Histogram();
    intermediate._collection.memory = new Measured.Histogram();

    aggregate._collection.cpu = new Measured.Histogram();
    aggregate._collection.memory = new Measured.Histogram();
  }
...
function runScenario(script, intermediate, aggregate) {
  ...
  if (!compiledScenarios) {
    ...
    scenarioEvents.on('response', function(delta, code) {
      // Call minigun-profiler and pass in all the variables
      if (script.pid) {
        return updateUsage(script.pid, intermediate, aggregate, pendingRequests, delta, code);
      }

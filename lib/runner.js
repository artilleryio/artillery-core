...
function runner(script, payload, options) {
  ...
  _.each(runnableScript.config.plugins, function tryToLoadPlugin(pluginConfig, pluginName) {
    let pluginConfigPackage = pluginConfig.package || runnableScript.config.pluginsPackage;
    let pluginPrefix = pluginConfigPackage ? pluginConfigPackage : 'artillery-plugin-';

...
HttpEngine.prototype.step = function step(requestSpec, ee, opts) {
  ...
  let f = function(context, callback) {
    ...
    if (config.http && config.http.proxy) {
      requestParams = _.extend(requestParams, {proxy: config.http.proxy});
    }
    ...
    if (params.body) {
      const file = fileReference(params.body, config);
      requestParams.body = file || template(params.body, context);
    }
    ...
    if (params.formData) {
      requestParams.formData = _.reduce(
        ...
        function (acc, v, k) {
          const reference = fileReference(v, config);
          if (reference) {
            acc[k] = reference;
          }

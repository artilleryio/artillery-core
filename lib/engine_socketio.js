...
function isResponseRequired(spec) {
  return (spec.emit && spec.emit.response.length);
}
...
SocketIoEngine.prototype.step = function (requestSpec, socketHeaders, ee) {
  ...
  let f = function(context, callback) {
    ...
    if (isResponseRequired(requestSpec)) {
      requestSpec.emit.response.forEach(rule => {
        let response = {
          channel: template(rule.channel, context),
          data: template(rule.data, context),
          capture: template(rule.capture, context),
          match: template(rule.match, context)
        };
